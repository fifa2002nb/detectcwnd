...
class Message(object):
    ...
    * tcpdir: The tcp.Direction corresponding to the HTTP message
    ...
    def __init__(self, tcpdir, pointer, msgclass):
        ...
        #self.ts_start_real = tcpdir.get_ts_by_index(1)
        #if not isinstance(self.ts_start_real,float):
        #    print ("%f %f %f\n" %( self.ts_start , self.ts_end, self.ts_start))
        #self.ts_start_real = self.ts_start
        #print ("%f %f %f\n" %( self.ts_start , self.ts_end, self.ts_start_real))
#        for i in range(self.seq_start,self.seq_end):
#            print ("%d %d\n" %( i,tcpdir.seq_final_arrival(i)))
#        sys.exit
