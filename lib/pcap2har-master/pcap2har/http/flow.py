...
class Flow(object):
    ...
    def __init__(self, tcpflow):
        ...
        success, requests, responses = parse_streams(tcpflow.fwd, tcpflow.rev, tcpflow)
        if not success:
            success, requests, responses = parse_streams(tcpflow.rev, tcpflow.fwd, tcpflow)
            if not success:
                # flow is not HTTP
...
def gather_messages(MessageClass, tcpdir, tcpflow):
    '''
    ...
    * tcpdir = TCPDirection, from which will be extracted the data
    ...
    while pointer < len(tcpdir.data):
        ...
        try:
			#edit by xuxia
            msg = MessageClass(tcpflow, pointer)
...
def parse_streams(request_stream, response_stream, tcpflow):
    '''
    ...
    try:
		#edit by xuxia
        requests = gather_messages(Request, request_stream, tcpflow)
        responses = gather_messages(Response, response_stream, tcpflow)
