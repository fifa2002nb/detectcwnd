...
class Direction(object):
    ...
    * final_arrival_pointer = the end sequence number of data that has
      completely arrived
	* tcp_pkg = syn/ack/syn+ack for the flow
    '''
    ...
    def __init__(self, flow):
        ...
        self.tcp_pkg = SortedCollection(key=itemgetter(0))
    ...
    def add(self, pkt):
        ...
        if pkt.data == '':
            self.tcp_pkg.insert((pkt.ts,pkt.ts))
            return
    ...
    def tcp_ts_by_ts_le(self, ts):
        '''
        Returns the time at which the seq number had fully arrived, that is,
        when all the data before it had also arrived.
        '''
        try:
            return self.tcp_pkg.find_le(ts)[1]
        except:
            return None

    def tcp_ts_by_ts_ge(self, ts):
        '''
        Returns the time at which the seq number had fully arrived, that is,
        when all the data before it had also arrived.
        '''
        try:
            return self.tcp_pkg.find_ge(ts)[1]
        except:
            return None


    def get_data_ts_by_index(self, index):
        '''
        Returns the time at which the seq number had fully arrived, that is,
        when all the data before it had also arrived.
        '''
        try:
            return self.final_arrival_data.find_by_index(index)[1]
        except:
            return None
