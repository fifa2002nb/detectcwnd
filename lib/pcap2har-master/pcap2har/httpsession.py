...
#print self.startedDateTime
#print request.ts_connect
        ...
        self.time_sending = (
            ms_from_dpkt_time_diff(request.ts_end_real, request.ts_start))
        if response is not None:
            self.time_waiting = (
                ms_from_dpkt_time_diff(response.ts_start, request.ts_end_real))
            self.time_receiving = (
        ...
        print(self.ts_start);
        print(request.ts_connect);
        print(request.ts_start);
        print(request.ts_end_real);
        print(response.ts_start);
        print(response.ts_end);
        print("-----");


        print(self.time_dnsing);
        print(self.time_connecting);
        print(self.time_sending);
        print(self.time_waiting);
        print(self.time_receiving);
    ...
    def add_dns(self, dns_query):
        ...
        if self.time_dnsing>-1:
            self.total_time += self.time_dnsing
...
class HttpSession(object):
    ...
    * entries = [Entry], all http request/response pairs
    ...
    def __init__(self, packetdispatcher):
        ...
        print "setting.process_pages"
        
        print settings.process_pages
